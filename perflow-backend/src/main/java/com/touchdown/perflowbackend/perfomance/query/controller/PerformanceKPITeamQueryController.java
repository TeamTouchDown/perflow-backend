...
public class PerformanceKPITeamQueryController {
    ...
    public ResponseEntity<KPIListResponseDTO> getNewTeamKPIs(
            @PathVariable(name = "empId") String empId,
        ...
        if (quarter == null && month == null) {
            // 년도별 조회 함수 호출
            response = KPIQueryService.getNewTeamKPIsByempId(empId, year);
        } else {
            // 기간별 조회 함수 호출
            response = KPIQueryService.getNewTeamKPIsempId(empId, year, quarter, month);
        }
        return ResponseEntity.ok(response);
    }
