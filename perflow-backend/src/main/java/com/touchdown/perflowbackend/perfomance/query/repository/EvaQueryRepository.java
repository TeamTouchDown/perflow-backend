...
public interface EvaQueryRepository extends JpaRepository<Perfo, Long> {
    ...
    @Query("SELECT new com.touchdown.perflowbackend.perfomance.query.dto.EvaAnswerResponseDTO( " +
            ...
            ") " +
            "FROM Perfo p " +
            "WHERE p.perfoEmp.empId = :perfoempId AND p.perfoedEmp.empId = :perfoedempId AND p.perfoQuestion.perfoType = 'COL' AND FUNCTION('YEAR', p.createDatetime) = :currentYear ")
